-- Made by seraph
-- FLUYEN HUB v3 - Giao diện VIP với thanh trạng thái, hiệu ứng màu luân hồi, bo góc, responsive
...
-- ===== LẤY THÔNG TIN PIN (mô phỏng nếu không có) =====
local BatteryPercent = 100
local function UpdateBattery()
    -- Thử lấy từ DeviceInfo nếu Roblox hỗ trợ, nếu không thì random
    local success, result = pcall(function()
        return game:GetService("DeviceInfo"):GetBatteryPercentage()
    end)
    if success and type(result) == "number" then
        BatteryPercent = math.floor(result * 100)
    else
        -- Mô phỏng pin giảm dần
        BatteryPercent = math.max(0, BatteryPercent - (math.random() > 0.9 and 1 or 0))
        if BatteryPercent < 5 then BatteryPercent = 100 end
    end
end
UpdateBattery()
task.spawn(function()
    while task.wait(30) do UpdateBattery() end
end)
...
local States = {
    ...
    Settings = { Theme = "Dark", Language = "English", Save = false, AutoLoad = false, Minimize = false, Draggable = true, ToggleKey = Enum.KeyCode.RightControl, Notifications = true }
}
...
-- Main Frame với bo góc và hiệu ứng màu
...
-- Bo góc (dùng UICorner)
local Corner = Instance.new("UICorner")
Corner.CornerRadius = UDim.new(0, 20)
Corner.Parent = MainFrame

-- Hiệu ứng màu luân hồi (dùng Shadow frame với gradient)
...
-- Tạo hiệu ứng màu luân hồi cho GlowFrame
local hue = 0
task.spawn(function()
    while true do
        hue = (hue + 0.005) % 1
...
-- Đảm bảo GlowFrame luôn theo MainFrame
...
ChangelogBtn.Size = UDim2.new(0, 100 * UIScale, 0, 30 * UIScale)
ChangelogBtn.Position = UDim2.new(0.9, 0, 0.4, 0)
ChangelogBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
ChangelogBtn.TextColor3 = Color3.new(1, 1, 1)
ChangelogBtn.Font = Enum.Font.Gotham
ChangelogBtn.TextSize = 14 * UIScale
ChangelogBtn.BorderSizePixel = 0
ChangelogBtn.Parent = TopBar
ChangelogBtn.MouseButton1Click:Connect(function()
    Notify("Changelog v3.0: Giao diện VIP, hiệu ứng màu luân hồi, thanh trạng thái, pin điện thoại")
end)
...
AddHomeLabel("Phiên bản: v3.0", homeY)
...
homeY = homeY + 30 * UIScale
AddHomeLabel("📜 Changelog: Giao diện VIP, hiệu ứng màu luân hồi", homeY)
homeY = homeY + 30 * UIScale
...
themeBtn.MouseButton1Click:Connect(function()
    ...
    themeLabel.Text = "🎨 Theme: " .. States.Settings.Theme
    local colors = {Dark = {bg = Color3.fromRGB(13,13,20), tab = Color3.fromRGB(25,25,35)}, Light = {bg = Color3.fromRGB(240,240,240), tab = Color3.fromRGB(220,220,220)}, ["Sky Blue"] = {bg = Color3.fromRGB(30,60,90), tab = Color3.fromRGB(40,80,120)}, Galaxy = {bg = Color3.fromRGB(10,5,30), tab = Color3.fromRGB(20,10,50)}}
...
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    if input.KeyCode == States.Settings.ToggleKey then
...
StatusBar.Size = UDim2.new(0.8, 0, 0, 45)
StatusBar.Position = UDim2.new(0.1, 0, 0.95, 0)
...
-- Hiệu ứng màu luân hồi cho thanh trạng thái (border glow)
local statusGlow = Instance.new("Frame")
statusGlow.Size = StatusBar.Size + UDim2.new(0, 6, 0, 6)
statusGlow.Position = StatusBar.Position - UDim2.new(0, 3, 0, 3)
statusGlow.BackgroundColor3 = Color3.fromRGB(0, 140, 255)
statusGlow.BackgroundTransparency = 0.4
statusGlow.BorderSizePixel = 0
statusGlow.Parent = StatusGui
local sgCorner = Instance.new("UICorner")
sgCorner.CornerRadius = UDim.new(0, 15)
sgCorner.Parent = statusGlow

task.spawn(function()
    while true do
        local hue = (tick() * 0.1) % 1
        statusGlow.BackgroundColor3 = Color3.fromHSV(hue, 0.8, 0.9)
        task.wait(0.02)
    end
end)
...
timeLabel.Position = UDim2.new(0.35, 0, 0, 0)
...
batteryLabel.Size = UDim2.new(0.2, 0, 1, 0)
batteryLabel.Position = UDim2.new(0.7, 0, 0, 0)
batteryLabel.BackgroundTransparency = 1
batteryLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
batteryLabel.Font = Enum.Font.Gotham
batteryLabel.TextSize = 18
batteryLabel.Parent = StatusBar

local keyLabel = Instance.new("TextLabel")
keyLabel.Size = UDim2.new(0.15, 0, 1, 0)
keyLabel.Position = UDim2.new(0.88, 0, 0, 0)
keyLabel.BackgroundTransparency = 1
keyLabel.TextColor3 = Color3.fromRGB(150, 150, 150)
keyLabel.Font = Enum.Font.Gotham
keyLabel.TextSize = 14
keyLabel.Text = "[ " .. tostring(States.Settings.ToggleKey.Name) .. " ]"
keyLabel.Parent = StatusBar
...
RunService.RenderStepped:Connect(function()
    ...
    -- Đổi màu pin theo mức
    if BatteryPercent < 20 then
        batteryLabel.TextColor3 = Color3.fromRGB(255, 50, 50)
    elseif BatteryPercent < 50 then
        batteryLabel.TextColor3 = Color3.fromRGB(255, 200, 50)
    else
        batteryLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
    end
end)
...
local function CreateKeyScreen()
    ...
    local Frame = Instance.new("Frame")
    Frame.Size = UDim2.new(0, 340 * UIScale, 0, 220 * UIScale)
    Frame.Position = UDim2.new(0.5, -170 * UIScale, 0.5, -110 * UIScale)
    ...
    local Title = Instance.new("TextLabel")
    Title.Text = "FLUYEN HUB"
    Title.Size = UDim2.new(1,0,0,50 * UIScale)
    ...
    Input.Size = UDim2.new(0.8,0,0,45 * UIScale)
    Input.Position = UDim2.new(0.1,0,0.35,0)
    Input.BackgroundColor3 = Color3.fromRGB(30,30,40)
    Input.TextColor3 = Color3.new(1,1,1)
    Input.Font = Enum.Font.Gotham
    Input.TextSize = 18 * UIScale
    Input.Parent = Frame
...
print("✅ FLUYEN HUB v3 - Giao diện VIP với thanh trạng thái, hiệu ứng màu luân hồi, pin điện thoại | [made by seraph]")